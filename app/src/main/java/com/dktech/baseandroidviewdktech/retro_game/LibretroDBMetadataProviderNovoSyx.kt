...
class LibretroDBMetadataProviderNovoSyx(private val mDatabase: LibretroDatabase) : GameMetadataProvider {
    companion object {
    ...
    override suspend fun retrieveMetadata(storageFile: StorageFile): GameMetadata? {
        ...
        val metadata =
            runCatching {
                findByCRC(storageFile, mDatabase)
                    ?: findBySerial(storageFile, mDatabase)
                    ?: findByFilename(mDatabase, storageFile)
                    ?: findByPathAndFilename(mDatabase, storageFile)
